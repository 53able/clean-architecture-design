...
Clean Architectureの原則を用いて、既存コードの設計診断、境界選定、段階的リファイン、依存境界レビューを支援するエージェントスキルです。
...
- [なぜこのスキルが必要か](why-clean-architecture-design.md): 変更コストと境界選定を中心に、このスキルの目的を説明します。
- [診断から段階的リファインまで](diagnosis-to-refinement.md): 観察、最小境界、port/adaptor、検証までの進め方です。
- [境界を作らない判断](when-not-to-use.md): 過剰な抽象化や安易なservice分割を避ける基準です。
